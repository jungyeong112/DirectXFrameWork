...
HRESULT CTexture::Initialize_Prototype(TEXTURE eType, const _tchar* pTextureFilePath, _uint iNumTextures)
{
	...
	{
		/* ../Bin/Resources/Textures/Explosion/Explosion(i).png */
		wsprintf(szTextureFilePath, pTextureFilePath, i);

		LPDIRECT3DBASETEXTURE9		pTexture = { nullptr };

		if (TEXTURE::PLANE == eType)
		{
			if (FAILED(D3DXCreateTextureFromFile(m_pGraphic_Device, szTextureFilePath, reinterpret_cast<LPDIRECT3DTEXTURE9*>(&pTexture))))
				return E_FAIL;
		}
		else
		{
			if (FAILED(D3DXCreateCubeTextureFromFile(m_pGraphic_Device, szTextureFilePath, reinterpret_cast<LPDIRECT3DCUBETEXTURE9*>(&pTexture))))
				return E_FAIL;
		}
